...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='.',
        help='output folder for girlseqrects-wcrt.npy and girlseq_frame*-wcrt.png',
    )
    ...
    for i in range(int(seq.shape[2])):
        ...
        if i == 0 or i==19 or i==39 or i==59 or i==79:
            ...
            ax.axis('off')
            plt.savefig(f'{output_folder}/girlseq_frame{i+1}-wcrt.png', bbox_inches='tight', pad_inches=0)
            #plt.show()

    np.save(f'{output_folder}/girlseqrects-wcrt.npy', rect_mat)
